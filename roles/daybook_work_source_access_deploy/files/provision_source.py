#!/usr/bin/env python3
"""Fail-closed provisioning of one fixed MinIO work source; secrets on stdin."""
import hashlib
import fcntl
import json
import os
...
POLICY_NAME = 'daybook-work-source-read'
...
def _provision(request, mc=None):
    ...
    if user is not None:
        ...
        return {'changed': False, 'category': 'exact_work_source_verified'}
    if owned:
    ...
    return {'changed': True, 'category': 'exact_work_source_created'}
...
def main():
    ...
    try:
        ...
        print(json.dumps({'changed': False, 'category': 'work_source_provisioning_refused'}))
        return 2
