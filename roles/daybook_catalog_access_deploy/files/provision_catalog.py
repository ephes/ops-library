#!/usr/bin/env python3
"""Fail-closed provisioning of the fixed MinIO catalogue writer; secrets on stdin.

The vault catalogue writes notes under `Katalog/` in the `obsidian` bucket and
nothing else: list only its three prefixes, get and put only below `Katalog/`,
and no delete. Everything else is the reusable work-source provisioner, unchanged.
"""
import hashlib
import fcntl
import json
...
POLICY_NAME = 'daybook-catalog-writer'
POLICY = {'Version': '2012-10-17', 'Statement': [
    {'Effect': 'Allow', 'Action': ['s3:ListBucket'], 'Resource': ['arn:aws:s3:::obsidian'],
     'Condition': {'StringEquals': {'s3:prefix': ['Katalog/matters/', 'Katalog/projects/',
                                                  'Katalog/writing/']}}},
...
def _provision(request, mc=None):
    ...
    if user is not None:
        ...
        return {'changed': False, 'category': 'exact_catalog_writer_verified'}
    if owned:
    ...
    return {'changed': True, 'category': 'exact_catalog_writer_created'}
...
def main():
    ...
    try:
        ...
        print(json.dumps({'changed': False, 'category': 'catalog_writer_provisioning_refused'}))
        return 2
