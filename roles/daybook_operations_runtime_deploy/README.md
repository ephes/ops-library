...
| `daybook_operations_runtime_code_bundle_src` | `""` (default), a path | A git bundle carrying the pinned revision, for a machine where no other role installs the code. The role then fetches every ref of it into a repository at `install_root/daybook` (`git init` plus a fetch, not a clone: the bundle carries main as `refs/remotes/origin/main`, which a clone would drop), checks out exactly `daybook_operations_runtime_revision` detached, refuses a checkout that differs, and runs `uv sync --frozen --no-dev` when `uv sync --check` says the environment is out of date. Replacing the checkout or changing its environment is refused while the supervisor label is loaded or an `operations serve` runs. Empty: the code is already installed (Studio's importer role does that). |
